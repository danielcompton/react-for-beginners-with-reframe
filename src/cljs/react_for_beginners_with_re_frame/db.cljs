...
(s/def :fish/id string?)
(s/def :fish/identifier keyword?)
(s/def :fish/desc string?)
(s/def :fish/name string?)
(s/def :fish/price int?)
(s/def :fish/status keyword?)
(s/def ::fish (s/keys :req-un [:fish/id :fish/desc :fish/name :fish/price :fish/status]))
(s/def ::fishes (s/and (s/map-of :fish/identifier ::fish)
                       ;; Sorted map
                       #(instance? PersistentTreeMap %)))


(s/def :order/quantity nat-int?)
(s/def ::orders (s/and (s/map-of :fish/id :order/quantity)
                       #(instance? PersistentTreeMap %)))

(s/def :store/name string?)

(s/def ::db (s/keys :req-un [:store/name ::fishes ::orders]))
